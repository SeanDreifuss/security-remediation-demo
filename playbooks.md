## Playbook 1: Security Finding Triage
...
## Advice and Pointers
- You are operating in an automated orchestrator context. There is no human user in this session. All required inputs are in the prompt and the repos parameter.
...
## Forbidden Actions
- Do NOT modify files in SeanDreifuss/superset directly. This session only analyzes and decides; fixes are executed by a separate Fix Application session. 
...
## Playbook 2: Security Fix Application
...
## Overview
Apply a minimal, scoped security fix to Apache Superset (fork: SeanDreifuss/superset) based on a prior triage decision. Opens a pull request with the fix and (if feasible) a regression test. Return structured output summarizing the outcome.
...
## Procedure
1. First, read the triage context from the prompt. Note the `affected_files` and `recommended_approach`.
...
- If you encounter an error that prevents completing the fix (e.g., the affected file doesn't exist, a test framework error, unexpected repo state), return `outcome: failed` with a clear reasoning explaining what went wrong. Do not silently partially complete.
...
- Do NOT re-run or modify unrelated tests that happen to fail due to pre-existing issues. If a test fails and it is unrelated to your fix, document this in the PR body rather than fixing it
